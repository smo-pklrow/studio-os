...
create table public.clients (
  ...
  start_date   date,
  due_date     date,
  buffer_tag   text,                   -- Buffer tag/label for this client
  share_token  text unique default gen_random_uuid()::text, -- client portal link
  created_at   timestamptz not null default now(),
...
-- ────────────────────────────────────────────────
-- TASK GROUPS  (Website, Orientation Campaign, etc.)
-- ────────────────────────────────────────────────
...
create table public.tasks (
  ...
  due_date        date,
  calendar_event_id text,               -- Google Calendar event ID once pushed
  sort_order      int  not null default 0,
  created_at      timestamptz not null default now(),
  updated_at      timestamptz not null default now()
);
...
-- ────────────────────────────────────────────────
-- TASK ASSIGNMENTS  (owner or subcontractor per task)
-- ────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────
-- BRAIN DUMP CARDS  (per client, freeform canvas)
-- ────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────
-- CLIENT CONTEXT  (AI memory per client — appended over time)
-- ────────────────────────────────────────────────
create table public.client_context (
  ...
  body        text not null,
  source      text,                     -- e.g. 'meeting_notes', 'task_complete', 'manual'
  created_at  timestamptz not null default now()
);
...
-- or via the CLI. Setting them up here as a reminder:
--
--   bucket: task-files
--     public: false
--     allowed MIME types: image/*, application/pdf, .docx, .figma
--
--   bucket: brain-dump-images
--     public: false
